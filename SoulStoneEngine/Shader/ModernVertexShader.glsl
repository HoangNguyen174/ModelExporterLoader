...
void main()
{
	...
	for( int i = 0; i < 4; i++ )
	{
		int boneIndex = a_vertexBoneIndex[i];
		pos += ( vec4( a_vertexPosition.xyz, 1.0 ) * u_boneTM[boneIndex] ) * a_vertexBoneWeight[i];
		norm += ( vec4( a_vertexNormal.xyz, 0.0 ) * u_boneTM[boneIndex] ) * a_vertexBoneWeight[i];
	}

	v_screenPosition = u_WorldToScreenMatrix * pos;

	vec4 finalPos = v_screenPosition * ( 1.0 - a_isStatic ) + v_screenPositionStatic * a_isStatic;
